...
import { installChildCoordination } from "./child-coordination.ts";
	...
	if (messages) {
		for (let i = messages.length - 1; i >= 0; i--) {
			...
			if (msg?.role === "assistant") {
				return !["aborted", "deferred", "pending"].includes(msg.stopReason);
			}
		}
	}
...
export default function (pi: ExtensionAPI) {
	const coordination = installChildCoordination(pi);
	...
	function renderWidget(ctx: { ui: { setWidget: (...args: any[]) => any } }, _theme: any) {
		ctx.ui.setWidget(
			"subagent-tools",
			(_tui: any, theme: any) => {
	...
	let parkedPhase: string | undefined;
	(pi as any).on("cache_warming_decision", () => (parkedPhase ? { action: "stop" } : undefined));
	const parkForPeers = async (ctx: any): Promise<boolean> => {
		if (!coordination) return false;
		try {
			const disposition = await coordination.disposition();
			if (!disposition.keepAlive) {
				parkedPhase = undefined;
				return false;
			}
			if (parkedPhase !== disposition.phase) {
				parkedPhase = disposition.phase;
				pi.appendEntry("subagent-run-wait", {
			...
			if (ctx.hasUI)
				ctx.ui.notify(`Peer lifecycle unavailable: ${(error as Error).message}`, "warning");
		}
		recorder.agentEndWaiting();
		return true;
	};
	const finishSettledRun = async (ctx: any) => {
		if (shutdownRequested) return;
		if (!findLatestAssistantError(lastRunMessages) && (await parkForPeers(ctx))) return;
		if (shutdownRequested || ctx.isIdle?.() === false || ctx.hasPendingMessages?.()) return;
		...
		if (shouldExit) {
			...
			pi.appendEntry("subagent-run-closure", {
				runId: process.env.PI_SUBAGENT_ID,
	...
	pi.on("agent_end", (event, ctx) => {
		lastRunMessages = (event as any).messages;
		// Older Pi lacks the settled boundary and the mode-bearing context.
		if ((ctx as any).mode === undefined) return finishSettledRun(ctx);
	});
	// Modern Pi can retry/continue after agent_end. Close only after settlement.
	(pi as any).on("agent_settled", (_event: unknown, ctx: any) => finishSettledRun(ctx));
	...
	(pi as any).on("after_provider_response", () => {
	...
	pi.registerTool({
		...
		description:
			...
			"In a peer review channel this parks the run while collaboration is pending; corrections and channel questions wake you automatically. Outside peer workflows it closes the session.",
		parameters: Type.Object({}),
		async execute(_toolCallId, _params, _signal, _onUpdate, ctx) {
			if (await parkForPeers(ctx))
				return {
					content: [
						{
							type: "text",
							text: "Parked for peer collaboration. No polling or coordinator resume needed.",
						},
					],
