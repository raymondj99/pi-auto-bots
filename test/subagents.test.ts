...
describe("subagent discovery", () => {
	...
	it("resolveEffectiveInteractive defaults to the inverse of auto-exit", () => {
		...
		// Bare assignments are autonomous; explicit iterate/fork remains interactive.
		...
		assert.equal(
			testApi.resolveEffectiveInteractive({ name: "A", task: "T", autoExit: false }, null),
			true,
		);
	});

	it("bare launch closes automatically without sacrificing explicit interactive or fork behavior", () => {
		assert.equal(
			testApi.resolveEffectiveAutoExit({ name: "Storage", task: "Implement" }, null),
			true,
		);
		assert.equal(
			testApi.resolveEffectiveAutoExit(
				{ name: "Storage", task: "Implement", interactive: false },
				null,
			),
			true,
		);
		assert.equal(
			testApi.resolveEffectiveAutoExit(
				{ name: "Interactive", task: "Discuss", interactive: true },
				null,
			),
			false,
		);
		assert.equal(
			testApi.resolveEffectiveAutoExit({ name: "Fork", task: "Iterate", fork: true }, null),
			false,
		);
		assert.equal(testApi.resolveEffectiveAutoExit({ name: "Planner", task: "Plan" }, {}), false);
		assert.equal(
			testApi.resolveEffectiveAutoExit(
				{ name: "Worker", task: "Implement", autoExit: true },
				{ autoExit: false },
			),
			true,
		);
	...
	it("buildSubagentToolAllowlist preserves requested tools and adds child control tools", () => {
		assert.equal(
			...
			"read,bash,web_search,caller_ping,subagent_done,subagent_team,subagent_task,subagent_channel,subagent_role,subagent_signal,subagent_artifact,subagent_permit,subagent_checkpoint",
		);
	});
...
describe("tool registration", () => {
	...
	it("restores the latest same-session resume identity by normalized session path", () => {
		const testApi = (subagentsModule as any).__test__;
		const entries = [
			{
				type: "custom",
				customType: "subagent-session-identity",
				data: {
					sessionId: "parent",
					sessionPath: "/tmp/agent.jsonl",
					memberId: "old-run",
					roleId: "reviewer",
					name: "Old name",
				},
		...
		const ctx = { sessionManager: { getSessionId: () => "parent", getEntries: () => entries } };
		assert.deepEqual(
			testApi.resolvePersistedSubagentIdentity(ctx, "/tmp/./agent.jsonl"),
			entries[2].data,
		);
		assert.equal(testApi.resolvePersistedSubagentIdentity(ctx, "/tmp/missing.jsonl"), undefined);
		assert.equal(
			testApi.validatePersistedSubagentIdentity({ ...entries[2].data, token: "ignored" })?.name,
			"Release reviewer",
		);
		assert.equal(
			testApi.validatePersistedSubagentIdentity({ ...entries[2].data, sessionPath: "" }),
			undefined,
		);
	});
});
...
describe("subagent interruption", () => {
	...
	it("delivers one final summary with separate pending workflow status", () => {
		const presentation = (subagentsModule as any).__test__.resolveResultPresentation(
			{
				exitCode: 0,
				elapsed: 12,
				summary: "Implemented and tested widget.",
				workflow: "task@3: review — awaiting coordinator review.",
				sessionFile: "/tmp/worker.jsonl",
			},
			"Worker",
		);
		assert.match(presentation, /Worker" finished/);
		assert.equal(presentation.match(/Implemented and tested widget/g)?.length, 1);
		assert.match(presentation, /awaiting coordinator review/);
